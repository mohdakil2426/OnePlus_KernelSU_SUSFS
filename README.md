...
Community-source `STOCK` builds remain clean upstream builds. Official `ONEPLUSOSS_*` STOCK builds apply the audited `oneplusoss-sm8250-strict-prototypes` compatibility set. It corrects two invalid function signatures and removes an unnecessary `__packed` function return-type annotation that prevents `genksyms` from generating one symbol version; none of these repairs changes runtime behavior. Their artifacts are therefore stock-derived, not byte-for-byte upstream. The selected defconfig remains unchanged. `KSUN` and `KSUN_SUSFS` add only the requested KernelSU-Next and SUSFS integrations.
